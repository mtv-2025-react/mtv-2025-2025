...
class Edit3DMMAdapter(nn.Module):
    def __init__(self, input_dim=50, hidden_dim=128,output_dim=25):
        """
        :param input_dim: 原始3DMM参数的维度
        :param vertex_dim: 顶点数量
        :param hidden_dim: 隐藏层维度
        """
        super(Edit3DMMAdapter, self).__init__()
    ...
    def forward(self, input_3dmm, change):
        # 将change_mask和change_magnitude展平并与input_3dmm拼接
        x = torch.cat([input_3dmm, change], dim=-1)
        ...
        delta = self.fc3(x)  # 预测需要添加的偏移量
        
        # 残差连接：原始3DMM + delta
        output_3dmm = input_3dmm + delta
...
class PriorLatentMatcher(BaseLatentModel):
    def __init__(self, conf, device):
        ...
        self.LDMMAdjuster_model=Edit3DMMAdapter()
    ...
    def _forward(
        ...
        if self.mode in ["train", "val"]: 
            ...
            with torch.no_grad():
                ...
                #model = AutoModel(model="emotion2vec_plus_large", device="gpu", ncpu=32, batch_size=s_audio_selected.size(0))
                #res = model.generate(s_audio_selected, output_dir="./outputs", granularity="utterance", extract_embedding=True)
                #s_audio_encodings = s_audio_encodings + res['embedding']


                s_audio_encodings = s_audio_encodings.repeat_interleave(self.k, dim=0)


                mean = torch.mean(l_emotion_selected, dim=1)  # shape=(4, 58)
                std = torch.std(l_emotion_selected, dim=1)    # shape=(4, 58)

                # 定义一个阈值
                threshold = 1.6 * std  # shape=(4, 58)

                # 计算每个样本和帧的偏差
                # 使用 unsqueeze(1) 来扩展维度以便广播
                deviations = torch.abs(l_emotion_selected - mean.unsqueeze(1))  # shape=(4, 50, 58)

                # 找出显著变化的位置
                # significant_changes = deviations > threshold.unsqueeze(1)  # shape=(4, 50, 58)

                # # 提取显著变化的顶点索引
                # vertex_indices_with_significant_change = torch.any(significant_changes, dim=1)  # shape=(4, 58)
                # vertices_with_significant_change = torch.where(vertex_indices_with_significant_change)

                # 推理
                adjusted = self.LDMMAdjuster_model(l_emotion_selected, deviations)
                x_start = self.latent_embedder.encode(adjusted).unsqueeze(1)  # (..., l_3dmm_selected)
